...
function paginate_links( $args = '' ) {
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		...
		$page_links[] = '<a class="prev page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . '<' . '</a>';
	endif;
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
	...
	if ( $args['prev_next'] && $current && ( $current < $total || -1 == $total ) ) :
		...
		$page_links[] = '<a class="next page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . '>' . '</a>';
	endif;
	switch ( $args['type'] ) {
		case 'array' :
			return $page_links;
